...
public final class Parser {
  ...
  private static void methodDecl() throws SourceFileErrorException
  {
    ...
    } else throw new SourceFileErrorException("Expected \"VOID\" or \"INT\" as second symbol, not" +
  ...
  private static void assignmentOrInvoc() throws SourceFileErrorException
  {
    ...
    /* XXXX*/
    accept(IDENT);
    if ((getCurrentToken() != LBRACKET) && (getCurrentToken() != BECOMES)) {
      argumentList();
      accept(SEMICOLON);
    }else {
      while (getCurrentToken() == LBRACKET) {
 ...
 | IDENTIFIER ( . nextInt '(' ')' | [<argumentList>]{'[' <expr3> ']'} )
  ...
  private static void expr1() throws SourceFileErrorException {
    ...
    switch (getCurrentToken()) {
      ...
      /* '(' <expr7> ')' */
      case LPAREN: nextToken(); expr7(); accept(RPAREN); break;

      /* (+|-|!) <expr1> */
      case PLUS:
      case MINUS:
      case NOT: nextToken();expr1(); break;
